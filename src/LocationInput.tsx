...
export function LocationInput({ onApply, disabled = false }: { onApply: (point: ResolvedLocation, mapsLink?: string) => void; disabled?: boolean }) {
  ...
  return <div className="location-input">
    <div className="location-input-tabs" role="group" aria-label="Location input method">
      ...
      <button type="button" className="btn" disabled={busy || disabled} aria-pressed={tab === 'link'} onClick={() => { setTab('link'); setError(null); }}>Google Maps link</button>
    </div>
    ...
    </div> : <label>Google Maps link or copied coordinates<input type="text" placeholder="Paste a link or 9.9312, 76.2673" value={link} disabled={busy || disabled} onChange={(event) => setLink(event.target.value)} /></label>}
    {error && <p className="error" role="alert">{error}</p>}
    <button type="button" className="btn" disabled={busy || disabled} onClick={() => void apply()}>{busy ? 'Reading location…' : 'Use this location'}</button>
  </div>;
}
